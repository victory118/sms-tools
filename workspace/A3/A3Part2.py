...
def optimalZeropad(x, fs, f):
    ...
    Ns = fs/f # number of samples per cycle of sinusoid
    M = np.size(x) # input signal length
    
    Ndft = int(np.ceil(M/Ns)*Ns)
    x_zeropad = np.zeros(Ndft)
    x_zeropad[:M] = x
    X = fft(x_zeropad)
    mX = 20*np.log10(abs(X[:(Ndft/2 + 1)]))
